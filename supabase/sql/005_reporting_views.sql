...
create or replace view public.v_source_progress_summary
with (security_invoker = true)
as
select
  src.id as source_id,
  ...
  count(*) filter (where a.status_code in ('draft', 'open', 'in_progress', 'pending_verification', 'on_hold')) as open_actions,
  count(*) filter (where a.status_code = 'closed') as closed_actions,
  count(*) filter (where a.status_code = 'verified') as verified_actions,
  count(*) filter (
  ...
  round(
    case
      ...
      else (count(*) filter (where a.status_code in ('closed', 'verified'))::numeric / count(a.id)::numeric) * 100
    end,
    2
  ) as closure_percent
...
left join public.source_types stp on stp.code = src.source_type_code
left join public.departments dept on dept.id = src.department_id
left join public.recommendations rec on rec.source_id = src.id
left join public.actions a on a.recommendation_id = rec.id
group by
  src.id,
  src.source_no,
  src.title,
