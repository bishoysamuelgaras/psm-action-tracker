...
drop function if exists public.reserve_source_number_v2(date, text) cascade;
drop function if exists public.reserve_source_number_v2(text, date) cascade;
drop function if exists public.reserve_source_number(text, date) cascade;
drop function if exists public.reserve_source_number(date, text) cascade;
drop function if exists public.reserve_recommendation_number_v2(uuid) cascade;
drop function if exists public.reserve_recommendation_number(uuid) cascade;
drop function if exists public.reserve_action_number_v2(uuid) cascade;
drop function if exists public.reserve_action_number(uuid) cascade;

create function public.reserve_source_number_v2(
  p_source_date date,
  p_source_type_code text
)
returns text
...
begin
  ...
  v_scope := p_source_type_code || ':' || v_year;

  select nr.reserved_no
  into v_existing
  from public.number_reservations nr
  where nr.entity_type = 'source'
    and nr.parent_scope = v_scope
    and nr.reserved_by = v_user
    and nr.is_consumed = false
    and nr.expires_at > timezone('utc', now())
  ...
  v_next := public.next_running_number('source_preview', v_scope);
  v_reserved_no := format('%s-%s-%s', v_prefix, v_year, lpad(v_next::text, 4, '0'));
...
create function public.reserve_source_number(
  p_source_type_code text,
  p_source_date date
)
returns text
...
set search_path = public
as $$
  select public.reserve_source_number_v2(p_source_date, p_source_type_code);
$$;

create function public.reserve_recommendation_number_v2(
  p_source_id uuid
)
returns text
language plpgsql
...
begin
  ...
  into v_existing
  from public.number_reservations nr
  where nr.entity_type = 'recommendation'
    and nr.parent_scope = p_source_id::text
    and nr.reserved_by = v_user
    and nr.is_consumed = false
    and nr.expires_at > timezone('utc', now())
  order by nr.created_at desc
  ...
  select s.source_no into v_source_no
  ...
  v_next := public.next_running_number('recommendation_preview', p_source_id::text);
  v_reserved_no := format('%s-R%s', v_source_no, lpad(v_next::text, 2, '0'));
...
create function public.reserve_recommendation_number(
  p_source_id uuid
)
returns text
language sql
...
set search_path = public
as $$
  select public.reserve_recommendation_number_v2(p_source_id);
$$;

create function public.reserve_action_number_v2(
...
begin
  ...
  into v_existing
  from public.number_reservations nr
  where nr.entity_type = 'action'
    and nr.parent_scope = p_recommendation_id::text
    and nr.reserved_by = v_user
    and nr.is_consumed = false
    and nr.expires_at > timezone('utc', now())
  order by nr.created_at desc
  ...
  select r.recommendation_no into v_recommendation_no
  ...
  v_next := public.next_running_number('action_preview', p_recommendation_id::text);
  v_reserved_no := format('%s-A%s', v_recommendation_no, lpad(v_next::text, 2, '0'));
...
create function public.reserve_action_number(
  p_recommendation_id uuid
)
returns text
language sql
...
set search_path = public
as $$
  select public.reserve_action_number_v2(p_recommendation_id);
$$;

grant execute on function public.reserve_source_number_v2(date, text) to authenticated;
grant execute on function public.reserve_source_number(text, date) to authenticated;
grant execute on function public.reserve_recommendation_number_v2(uuid) to authenticated;
grant execute on function public.reserve_recommendation_number(uuid) to authenticated;
grant execute on function public.reserve_action_number_v2(uuid) to authenticated;
grant execute on function public.reserve_action_number(uuid) to authenticated;
